...
$sql = "SELECT id_pengguna FROM jadwal_book_xl where id_pengguna='$id_pengguna' and tanggal='$tanggal'";
...
if ($result->num_rows > 0) {
        ...
        echo "<br />";
			...
			$result=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT count(*) as total from jadwal_book_xl where tanggal='$tanggal' AND jam_kelas='$jam_kelas'");
            $data=mysqli_fetch_assoc($result);
             $total    = $data['total'];

             if ($total < 25) {
             	 // jika murid yang memilih kelas di bawah 5 pada tanggal dan jam yang di tentukan di atas
             	$query = "INSERT INTO jadwal_book_xl(id_guru,id_pengguna,jam_kelas,tanggal,nama_guru,jenis_kelas) VALUES ('".$id_guru."','".$id_pengguna."','".$jam_kelas."','".$tanggal."','".$nama_guru."','".$selected."') ";
