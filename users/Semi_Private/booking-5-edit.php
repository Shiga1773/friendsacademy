         ...
         <!-- Page Heading -->
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
                            <li>
                                <i class="fa fa-file"></i> Semi - Private Class
                            </li>
                         </ol>
                         </div>
                </div>
...
foreach($_POST['check_list2'] as $selected){
                        ...
                        $query = "SELECT * FROM pengguna where id ='$id_user2' "; // Tampilkan semua data gambar
                        $sql = mysqli_query($connect, $query); // Eksekusi/Jalankan query dari variabel $query
                        $row = mysqli_num_rows($sql); // Ambil jumlah data dari hasil eksekusi $sql
...
<!-- <input type='' name='' value=''> -->
    ...
    <form name="myForm" method="post" onsubmit="return validateForm()" enctype="multipart/form-data" target="_blank" action="../../action/tambah_kelas_book_m.php"  >
        <?php 
                    ...
                    <thead>
                        <tr>
                                ...
                                <td>
                                    ...
                                    <input type="hidden" name="id_user2" id="id_user2" value="<?php print($id_guru) ?>  " >
    ...
    <option disabled selected>-- Pick a Date --</option>
     <?php
                    // Load file koneksi.php
                        ...
                        $query = "SELECT DISTINCT tanggal FROM jadwal_kelas WHERE id_guru='$selected' AND jenis_kelas='m' "; // Tampilkan semua data gambar
                        $sql = mysqli_query($connect, $query); // Eksekusi/Jalankan query dari variabel $query
                        $row = mysqli_num_rows($sql); // Ambil jumlah data dari hasil eksekusi $sql
                        if($row > 0){ // Jika jumlah data lebih dari 0 (Berarti jika data ada)
                        while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
                        ...
                        ?> </select> <button type="submit" class="btn btn-sm btn-info" formaction="booking-5.php">Cek Jam Kelas</button>
    ...
    <option disabled selected>-- Pick a Date --</option>
     <?php
                    // Load file koneksi.php
                        ...
                        $query = "SELECT DISTINCT jam_kelas FROM jadwal_kelas WHERE id_guru='$id_user2' AND jenis_kelas='m' and tanggal='$tanggal' "; // Tampilkan semua data gambar
                        $sql = mysqli_query($connect, $query); // Eksekusi/Jalankan query dari variabel $query
                        $row = mysqli_num_rows($sql); // Ambil jumlah data dari hasil eksekusi $sql
                        if($row > 0){ // Jika jumlah data lebih dari 0 (Berarti jika data ada)
                        while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
                    ...
                    // Load file koneksi.php
                        ...
                        $query = "SELECT * FROM jadwal_kelas WHERE id_guru='$selected' AND jenis_kelas='m' AND tanggal='$tanggal_a' "; // Tampilkan semua data gambar
                        $sql = mysqli_query($connect, $query); // Eksekusi/Jalankan query dari variabel $query
                        $row = mysqli_num_rows($sql); // Ambil jumlah data dari hasil eksekusi $sql
                        if($row > 0){ // Jika jumlah data lebih dari 0 (Berarti jika data ada)
                        while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
                        ...
                        echo "
                        <input type='text' name='id_kelas' id='id_kelas' value='". $data['id'] ."'>
