...
<div class="rowa">
                        ...
                        $query = "SELECT * FROM jadwal_kelas WHERE id_guru='$selected' AND jenis_kelas='s1' AND nama_kelas='private' "; // Tampilkan semua data gambar
                        $sql = mysqli_query($connect, $query); // Eksekusi/Jalankan query dari variabel $query
                        $row = mysqli_num_rows($sql); // Ambil jumlah data dari hasil eksekusi $sql
    ...
    <input type='hidden' name='id_pengguna' id='id_pengguna' value='$id_user' >
      ...
      <p> <button type='submit' class='btn btn-sm btn-info'  name='submit' id='more_d' formaction='../../action/tambah_kelas_book.php' style='width:100%;'><label><input name='check_list2[]'  type='radio' value='".$data['jenis_kelas']."' style='visibility:hidden'>Book&nbsp;&nbsp;</label></button></p>
    </div>
  </div>                 
