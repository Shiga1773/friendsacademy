	...
	?>
        ...
        <div id="page-wrapper">
         ...
         <!-- Page Heading -->
            ...
            <form action="booked_class_m.php" method="post" enctype="multipart/form-data" target="_blank">
    ...
    <option disabled selected>-- Pick a Date --</option>
     <?php
                    // Load file koneksi.php
                        ...
                        $query = "SELECT DISTINCT tanggal FROM jadwal_kelas WHERE id_guru='$id_user' AND jenis_kelas='m' "; // Tampilkan semua data gambar
    ...
    <option disabled selected>-- Select Time --</option>
     <?php
                    // Load file koneksi.php
                        ...
                        $query = "SELECT DISTINCT jam_kelas FROM jadwal_kelas WHERE id_guru='$id_user' AND jenis_kelas='m' "; // Tampilkan semua data gambar
                        $sql = mysqli_query($connect, $query); // Eksekusi/Jalankan query dari variabel $query
                        $row = mysqli_num_rows($sql); // Ambil jumlah data dari hasil eksekusi $sql
                        if($row > 0){ // Jika jumlah data lebih dari 0 (Berarti jika data ada)
                        while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
...
<table class="table table-bordered table-hover table-striped">
 ...
 <br> <br>
             ...
             <table border='0' width='100%'>
                        ...
                        $query = "SELECT DISTINCT * FROM pengguna inner join jadwal_book_m on pengguna.id = jadwal_book_m.id_pengguna WHERE jadwal_book_m.tanggal='$tanggal' AND jadwal_book_m.jam_kelas='$jam_kelas' ORDER BY tanggal ASC "; // Tampilkana semua data gambar
                        $sql = mysqli_query($connect, $query); // Eksekusi/Jalankan query dari variabel $query
                        $row = mysqli_num_rows($sql); // Ambil jumlah data dari hasil eksekusi $sql
