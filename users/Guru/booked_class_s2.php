...
<?php
    ...
    <form action="booked_class.php" method="post" enctype="multipart/form-data" target="_blank">
    ...
    <option disabled selected>-- Pick a Date --</option>
     <?php
                    // Load file koneksi.php
                        ...
                        $query = "SELECT DISTINCT tanggal FROM jadwal_kelas WHERE id_guru='$id_user' AND jenis_kelas='s2' "; // Tampilkan semua data gambar
...
<div class="rowa">
             <table border='0' width='100%'>
                        ...
                        $query = "SELECT DISTINCT * FROM pengguna inner join jadwal_book on pengguna.id = jadwal_book.id_pengguna WHERE jadwal_book.tanggal='$tanggal' ORDER BY tanggal ASC "; // Tampilkana semua data gambar
                        $sql = mysqli_query($connect, $query); // Eksekusi/Jalankan query dari variabel $query
                        $row = mysqli_num_rows($sql); // Ambil jumlah data dari hasil eksekusi $sql
    ...
    <div class='card'>
     ...
     <h2>".$data['nama2']."</h2>
     <p>".$data['jam_kelas']." / ".$data['jam_kelas_b']."</p>  
